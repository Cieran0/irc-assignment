...
int bot::main(const std::string_view& program, const std::vector<std::string_view>& arguments) {
    ...
    while (bot::isAlive){
        ...
        //parseCommand failed
        if(recievedCommand.command.empty()) {
...
void bot::handleCommand(irc::command commandToHandle, bot::details botDetails, bot::clientSocket botSocket ) {
    if(commandToHandle.command.starts_with(":")) 
    {
        std::string hostnameOrUserNickname = commandToHandle.command.substr(1,commandToHandle.command.length()-1);
    ...
    else if (commandToHandle.command == "PING") 
    {
    ...
    {
        std::cout << "Uknown command: "<< commandToHandle.command << std::endl;
    }
}